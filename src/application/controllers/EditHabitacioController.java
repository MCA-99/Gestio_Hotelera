...
public class EditHabitacioController {
	private Habitacio habitacioEdit;
	...
	/*Constructor*/
	public EditHabitacioController() {
		this.habitacioEdit = GestioHabitacioController.habitacioEdit;
	}
    ...
    void guardar(ActionEvent event) {
    	if(!planta.getText().isEmpty() && !preu.getText().isEmpty() && !estat.getSelectionModel().isEmpty() && !tipus.getSelectionModel().isEmpty() && !descripcio.getText().isEmpty() && !caracteristiques.getText().isEmpty()){
    		error.setText("");
    		habitacioEdit.update(habitacioEdit.getId_habitacio(), Integer.parseInt(planta.getText()), Float.parseFloat(preu.getText()), estat.getSelectionModel().getSelectedItem(), tipus.getSelectionModel().getSelectedItem(), descripcio.getText(), caracteristiques.getText());
    ...
    @FXML
    void initialize() {
    	planta.setText(Integer.toString(habitacioEdit.getPlanta()));
    	preu.setText(Float.toString(habitacioEdit.getPreu()));
    	estat.setItems(estat_list);
    	estat.getSelectionModel().select(habitacioEdit.getEstat());
    	tipus.setItems(tipus_list);
    	tipus.getSelectionModel().select(habitacioEdit.getTipus());
    	descripcio.setText(habitacioEdit.getDescripcio());
    	caracteristiques.setText(habitacioEdit.getCaracteristiques());
    }
